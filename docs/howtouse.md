...
Scripts can be run under a debugger, so a host application (an IDE, a rule editor, a REPL) can offer **breakpoints**, **stepping**, a **call stack**, and live **variable/watch evaluation** while execution is paused. The library provides the hooks; the UI is up to you.

```csharp
var context  = new ParserContext { AssignmentPermissions = AssignmentPermissions.All };
var debugger = new ScriptDebugger();
context.Debugger = debugger;                 // enable debugging for this execution

debugger.Breakpoints.Add(3);                 // break on line 3
debugger.Break += (sender, e) =>
{
    Console.WriteLine($"Paused at line {e.Location.Start.Line}: total = {e.Evaluate("total")}");
