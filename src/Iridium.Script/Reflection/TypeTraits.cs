...
internal class TypeTraits
{
    ...
    private readonly TypeTraits _elementTypeInspector;
    ...
    public TypeTraits(Type type)
    {
        ...
        if (type.IsArray)
            _elementTypeInspector = new TypeTraits(type.GetElementType());
    ...
    private TypeTraitFlags BuildTypeFlags()
    {
        ...
        else if (Type.IsArray)
        {
            ...
            if ((_elementTypeInspector.TraitFlags & TypeTraitFlags.CanBeNull) != 0)
                flags |= TypeTraitFlags.ElementCanBeNull;
            if ((_elementTypeInspector.TraitFlags & TypeTraitFlags.Nullable) != 0)
                flags |= TypeTraitFlags.ElementNullable;
            if ((_elementTypeInspector.TraitFlags & TypeTraitFlags.ValueType) != 0)
